...
function ApiProvider({ children }) {
  ...
  const fetchData = async (endpoint, type) => {
    ...
    try {
      ...
      setData((prevData) => ({
        ...prevData,
        [type]: [...prevData[type], ...jsonData.results]
      }))
    } catch (error) {
  ...
  useEffect(() => {
    fetchData(`https://api.themoviedb.org/3/trending/all/day?api_key=${API_KEY}&page=${page}`, "trendings")
    fetchData(`https://api.themoviedb.org/3/movie/top_rated?api_key=${API_KEY}&page=${page}`, "movies")
    fetchData(`https://api.themoviedb.org/3/tv/top_rated?api_key=${API_KEY}&page=${page}`, "series")
  }, [])
